...
class Window(QMainWindow, Ui_MainWindow):
    def __init__(self):
        ...
        self.statusBar.showMessage("Please select a file")
    ...
    def on_openAct_triggered(self):
        ...
        imgPath = QFileDialog.getOpenFileName(
            self, "Please select an image", "/", "All Files (*)")[0]
    ...
    def openImage(self, imgPath):
        ...
        if imgPath != '':
            ...
            QMessageBox.warning(self, "Notice", "Please reselect an image")

    def historyFile(self):
        with open(self.history_path, 'r+') as fp:  # Open in read-write mode
            history = fp.readlines()
            history = self.imgPath + '\n' + history[0] + history[1]  # Keep only last 3 records
            fp.seek(0)  # Move cursor to the beginning
            fp.truncate()  # Clear existing content
            fp.write(history)  # Write updated history
    ...
    def on_workThread_finishSignal(self, T, R):
        self.T = T
        self.R = R
        self.statusBar.showMessage("Current image path: " + self.imgPath + "   Image Enhancement Successful")
    ...
    def on_saveAsAct_triggered(self):
        savePath = QFileDialog.getSaveFileName(
            self, "Please choose a save location", "/", "BMP Format (.bmp);; JPG Format (.jpg)")[0]
        if savePath != '':
            imsave(savePath, self.R)
            QMessageBox.about(self, "Notice", "Saved successfully")
        else:
            QMessageBox.warning(self, "Notice", "Please reselect the save location")
    ...
    def on_denoiseAct_triggered(self):
        ...
        self.enhancedImgFigure.axes.imshow(self.R)
        self.enhancedImgFigure.draw()
        QMessageBox.about(self, "Notice", "Denoising successful")
